...
class LoadImages:
    ...
    def __init__(self, path, img_size=640, stride=32, auto=True, transforms=None, vid_stride=1):
        ...
        for p in sorted(path) if isinstance(path, (list, tuple)) else [path]:
            p = str(Path(p).resolve()) #同样是获取绝对路径
            if "*" in p:
        ...
        images = [x for x in files if x.split(".")[-1].lower() in IMG_FORMATS]  #看看格式是否正确
        videos = [x for x in files if x.split(".")[-1].lower() in VID_FORMATS]
        ni, nv = len(images), len(videos)  #图片数量 和 视频数量
    ...
    def __next__(self):
        ...
        if self.count == self.nf:  #如果count == 文件总数
            raise StopIteration    #识别完了 返回
        ...
        if self.transforms:
            im = self.transforms(im0)  # transforms    把图片设置成能输入的大小 stride的倍数
        else:
            im = letterbox(im0, self.img_size, stride=self.stride, auto=self.auto)[0]  # padded resize
            im = im.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB
            im = np.ascontiguousarray(im)  # contiguous
