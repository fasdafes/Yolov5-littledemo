...
def train(hyp, opt, device, callbacks):
    ...
    (w.parent if evolve else w).mkdir(parents=True, exist_ok=True)  # 如果不存在weights文件夹就创建一个
    last, best = w / "last.pt", w / "best.pt"   #选出最好的一个weights
    ...
    init_seeds(opt.seed + 1 + RANK, deterministic=True) #随机种子 确定计算机实验可复现
    ...
    pretrained = weights.endswith(".pt")  #使用了预训练权重
    if pretrained:
        with torch_distributed_zero_first(LOCAL_RANK):
            weights = attempt_download(weights)  # download if not found locally
        ckpt = torch.load(weights, map_location="cpu")  # load checkpoint to CPU to avoid CUDA memory leak
        model = Model(cfg or ckpt["model"].yaml, ch=3, nc=nc, anchors=hyp.get("anchors")).to(device)  # 使用预训练权重创建新网络，为什么还要新建一个模型，因为nc可能不同
...
def main(opt, callbacks=Callbacks()):
    ...
    if opt.resume and not check_comet_resume(opt) and not opt.evolve: #resume就是中断继续
        ...
        assert len(opt.cfg) or len(opt.weights), "either --cfg or --weights must be specified" #如果即没有告诉train如何搭建网络，也没有给训练好的weight，则报错
        if opt.evolve:  #训练的时候会使用很多参数，这个就是进化超参数。意思就是模型会自己尝试多次不同超参数的训练过程，最后给出一个最好的，自动搜索更优解空间中的超参数组合，让你的训练结果更好、更稳、更快收敛
            if opt.project == str(ROOT / "runs/train"):  # if default project name, rename to runs/evolve
                opt.project = str(ROOT / "runs/evolve")
            opt.exist_ok, opt.resume = opt.resume, False  # pass resume to exist_ok and disable resume
        if opt.name == "cfg":  #如果传入name 就把name设置成cfg对应的值
    ...
    # Train
    if not opt.evolve:
        train(opt.hyp, opt, device, callbacks) #模型训练
