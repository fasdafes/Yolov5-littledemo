...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights="yolov5s.pt", device=torch.device("cpu"), dnn=False, data=None, fp16=False, fuse=True):
        ...
        super().__init__()
        w = str(weights[0] if isinstance(weights, list) else weights) #如果权重是一个列表的话就以第一个作为地址，如果不是就是传进来的那个地址
        pt, jit, onnx, xml, engine, coreml, saved_model, pb, tflite, edgetpu, tfjs, paddle, triton = self._model_type(w)#检查一下是什么类型的模型pt就是pytorch
        fp16 &= pt or jit or onnx or engine or triton  # FP16
        nhwc = coreml or saved_model or pb or tflite or edgetpu  # 是 YOLOv5 的模型推理部分中用来判断输入/输出的张量格式是否是 NHWC 格式（也叫 BHWC）的一行逻辑判断。而 PyTorch 使用的是 NCHW = [Batch, Channels, Height, Width]
        ...
        if pt:  # PyTorch
            model = attempt_load(weights if isinstance(weights, list) else w, device=device, inplace=True, fuse=fuse) #加载模型
            stride = max(int(model.stride.max()), 32)  # 设定步长
